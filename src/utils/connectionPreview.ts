...
function shouldPreviewCandidate(activeType: PinType | undefined, candidateType: PinType): boolean {
  if (!activeType) {
    return false;
  }
  if (activeType === 'execution') {
    return candidateType === 'execution';
  }
  if (candidateType === 'execution') {
    return false;
  }
  return true;
}
...
export function buildConnectionPreviewForNode(
  ...
  if (active.handleType === 'source') {
    ...
    for (const pin of node.data.inputs || []) {
      if (!shouldPreviewCandidate(active.pinType, pin.type)) continue;
