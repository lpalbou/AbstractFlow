...
export function RunFlowModal({
  ...
  const [voiceWaitUploadedRef, setVoiceWaitUploadedRef] = useState<CanonicalArtifactRef | null>(null);
  const voiceWaitRecorderRef = useRef<MediaRecorder | null>(null);
  const voiceWaitStreamRef = useRef<MediaStream | null>(null);
  const voiceWaitChunksRef = useRef<Blob[]>([]);
  const voiceWaitDiscardRef = useRef(false);
  ...
  const isVoiceInputWait = useMemo(() => {
    const details = waitingPayload?.details && typeof waitingPayload.details === 'object'
      ? (waitingPayload.details as Record<string, unknown>)
      : {};
    const inputMode = typeof details.input_mode === 'string' ? details.input_mode.trim().toLowerCase() : '';
    return inputMode === 'voice' || selectedStep?.nodeType === 'listen_voice';
  }, [selectedStep?.nodeType, waitingPayload?.details]);

  const stopVoiceWaitStream = useCallback(() => {
    const stream = voiceWaitStreamRef.current;
    voiceWaitStreamRef.current = null;
    if (stream) {
      for (const track of stream.getTracks()) track.stop();
    }
  }, []);

  const uploadVoiceWaitBlob = useCallback(
    async (blob: Blob, filename: string): Promise<CanonicalArtifactRef> => {
      const sessionId = artifactSessionId.trim() || derivedSessionId || stableSessionId || '';
      if (!sessionId) throw new Error('A session id is required to upload captured audio.');
      const uploadDescriptor = gatewayContracts?.common?.attachments?.upload;
      if (!descriptorEndpointAvailable(uploadDescriptor)) {
        throw new Error('Gateway attachment upload is not available.');
      }
      ...
      if (!ref) throw new Error('Gateway upload did not return an audio artifact reference.');
      return ref;
    },
    [artifactSessionId, derivedSessionId, gatewayContracts?.common?.attachments?.upload, stableSessionId]
  );

  const resumeVoiceWaitWithArtifact = useCallback(
    async (ref: CanonicalArtifactRef) => {
      if (resumeSubmitting) return;
      setResumeSubmitting(true);
  ...
  const startVoiceWaitRecording = useCallback(async () => {
    ...
    if (typeof MediaRecorder === 'undefined') {
      setVoiceWaitError('This browser does not expose MediaRecorder.');
      return;
    }
    setVoiceWaitError(null);
    setVoiceWaitUploadedRef(null);
    try {
      ...
      voiceWaitDiscardRef.current = false;
      voiceWaitChunksRef.current = [];
      voiceWaitStreamRef.current = stream;
      voiceWaitRecorderRef.current = recorder;
      recorder.ondataavailable = (event) => {
        if (event.data && event.data.size > 0) voiceWaitChunksRef.current.push(event.data);
      };
      recorder.onerror = (event) => {
        setVoiceWaitError((event as any)?.error?.message || 'Recording failed.');
      };
      recorder.onstop = async () => {
        const chunks = voiceWaitChunksRef.current;
        const discard = voiceWaitDiscardRef.current;
        voiceWaitDiscardRef.current = false;
        voiceWaitChunksRef.current = [];
        setVoiceWaitRecording(false);
        stopVoiceWaitStream();
        if (discard) {
          setVoiceWaitBusy(false);
          voiceWaitRecorderRef.current = null;
          return;
        }
        if (!chunks.length) {
          setVoiceWaitBusy(false);
          setVoiceWaitError('No audio was captured.');
          return;
        }
        setVoiceWaitBusy(true);
        try {
          const recordedType = recorder.mimeType || mimeType || 'audio/webm';
          const ext = recordedType.includes('mp4') ? 'm4a' : recordedType.includes('wav') ? 'wav' : 'webm';
          const blob = new Blob(chunks, { type: recordedType });
          const ref = await uploadVoiceWaitBlob(blob, `voice-response.${ext}`);
          setVoiceWaitUploadedRef(ref);
          await resumeVoiceWaitWithArtifact(ref);
        } catch (error) {
          setVoiceWaitError(error instanceof Error ? error.message : 'Failed to upload recorded audio.');
        } finally {
          setVoiceWaitBusy(false);
          voiceWaitRecorderRef.current = null;
        }
      ...
      recorder.start();
      setVoiceWaitRecording(true);
    } catch (error) {
      stopVoiceWaitStream();
      setVoiceWaitRecording(false);
      setVoiceWaitBusy(false);
      setVoiceWaitError(error instanceof Error ? error.message : 'Microphone recording failed.');
    }
  }, [resumeSubmitting, resumeVoiceWaitWithArtifact, stopVoiceWaitStream, uploadVoiceWaitBlob, voiceWaitBusy, voiceWaitRecording]);

  const stopVoiceWaitRecording = useCallback(() => {
    const recorder = voiceWaitRecorderRef.current;
  ...
  useEffect(() => {
    if (!isVoiceInputWait) return;
    return () => {
      const recorder = voiceWaitRecorderRef.current;
      if (recorder && recorder.state !== 'inactive') {
        try {
          voiceWaitDiscardRef.current = true;
          recorder.stop();
        } catch {
          // Best-effort cleanup.
        }
  ...
  }, [isVoiceInputWait, stopVoiceWaitStream, waitingKey]);
                  ...
                  <div className="run-details-header-badges">
                    ...
                    isSubworkflowWait ? (
                      ...
                      <div className="run-waiting">
                        <div className="run-waiting-prompt">
                          <MarkdownRenderer
                            markdown={(waitingPayload?.prompt || 'Speak now.').trim()}
                          />
                        </div>
                        <div className="run-voice-wait-panel">
                          <div className="run-voice-wait-status">
                            <span className={voiceWaitRecording ? 'recording' : voiceWaitBusy ? 'busy' : ''} />
                            {voiceWaitRecording
                              ? 'Recording...'
                              : voiceWaitBusy || resumeSubmitting
                                ? 'Uploading audio...'
                          ...
                          <div className="run-voice-wait-actions">
                            {!voiceWaitRecording ? (
                              ...
                              <button
                                ...
                                onClick={stopVoiceWaitRecording}
                                disabled={voiceWaitBusy}
                              >
                                Stop and continue
                              </button>
                            )}
