  ...
  loadFlow: (flow) => {
    ...
    const nodes: Node<FlowNodeData>[] = flowNodes.map((vn) => {
      ...
      if (['generate_image', 'edit_image', 'image_to_image', 'generate_video', 'text_to_video', 'image_to_video', 'generate_voice', 'generate_music', 'transcribe_audio', 'listen_voice'].includes(data.nodeType)) {
        ...
        const isLegacyAutoImageSize = (defaults: Record<string, unknown>, provider: string, model: string): boolean =>
        ...
        if (data.nodeType === 'generate_voice') {
          ...
          if (isLegacyAutoImageSize(nextDefaults, provider, model)) {
            delete nextDefaults.width;
            delete nextDefaults.height;
            changedDefaults = true;
          }
