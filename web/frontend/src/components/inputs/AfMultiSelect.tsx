...
import type { AfSelectOption } from './AfSelect';

export interface AfMultiSelectProps {
  values: string[];
...
export function AfMultiSelect({
  ...
  useEffect(() => {
    if (!open) return;
    // Highlight first selected option when opening (if present).
    let idx = -1;
    for (let i = 0; i < filtered.length; i++) {
      if (selected.has(filtered[i].value)) {
        idx = i;
        break;
      }
    }
  ...
  }, [open, filtered, searchable, selected]);
  ...
  const showValue = selectedLabels.length > 0;
  const triggerText = showValue
